...
school_result.pca <- prcomp(school_result, scale. = TRUE)
ggscreeplot(school_result.pca)
...
# This single-link merge criterion is local. We pay attention solely to the area where the two clusters come closest to each other. 
...
# This is equivalent to choosing the cluster pair whose merge has the smallest diameter. This complete-link merge criterion is non-local; 
# the entire structure of the clustering can influence merge decisions. This results in a preference for compact clusters with small diameters over long, straggly clusters, but also causes sensitivity to outliers.
...
memb_single <- cutree(crime_single, 5)
plot(crime_single)
rect.hclust(crime_single, k = 5, border = 2:6)
...
print(dunn_km) # 0.1658424
print(dunn_single) # 0.2400915
...
# I will offer single-linkage hierarchical clustering to my client
