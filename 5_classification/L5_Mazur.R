...
sum(is.na(full$Age))  # The result is 86, so all missing values are represented by NA
...
tbl_train <- table(actual_train$Survived, pred_train$predict)
(accuracy_train <- sum(diag(tbl_train)) / sum(tbl_train)) 
# 0.001420455 снова очень мальнькие значения
...
# 3) Try Stacked Ensemble with GLM, RF, GBM and XGBoost ?????выдает ошибку
...
factors <- c("Pclass", "Survived")
train %<>% mutate_at(factors, as.factor)
...
tbl_train <- table(actual_train$Survived, pred_train$predict)
(accuracy_train <- sum(diag(tbl_train)) / sum(tbl_train)) 
# очень долго загружалось и в итоге зависало, так произошло 3 раза
tbl_test <- table(actual_test$Survived, pred_test$predict)
(accuracy_test <- sum(diag(tbl_test)) / sum(tbl_test)) 
#  
